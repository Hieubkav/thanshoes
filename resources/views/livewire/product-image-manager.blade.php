...
>
    ...
    <div class="grid grid-cols-3 gap-4 md:grid-cols-4 lg:grid-cols-6" x-ref="imageList">
        ...
        <div data-id="{{ $image->id }}" class="group relative aspect-square rounded-lg bg-gray-100 shadow-sm">
            ...
            <div class="absolute left-2 top-2 rounded bg-primary-500 px-1.5 py-0.5 text-xs font-medium text-white">
                Variant
            </div>
            @endif
        </div>
        ...
        <div class="mt-4">
            <button type="button"
                    @click="
                        ...
                        $wire.addFromVariantImages(selected)
                    "
                    class="inline-flex items-center rounded-lg bg-primary-600 px-4 py-2 text-white shadow hover:bg-primary-500">
                Thêm ảnh đã chọn
            </button>
...
document.addEventListener('livewire:load', function () {
    const el = document.querySelector('[x-ref="imageList"]')
    if (el) {
        new Sortable(el, {
            animation: 150,
            handle: '.drag-handle',
